...
Portfolio task `DGN-1786264762917684` owns this change. Model TLS fingerprint and same-SNI connection concurrency as independent evidence axes so diagnostics can identify their conjunction without adding another failure-symptom signal

## Goals / Non-Goals

- Goal: deliver `Add connection-concurrency as an independent evidence axis` with observable evidence.
