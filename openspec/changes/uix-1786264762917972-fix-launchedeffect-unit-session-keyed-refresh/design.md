...
Portfolio task `UIX-1786264762917972` owns this change. The 2026-06-10 Compose audit found three LaunchedEffect(Unit) sites that drive ViewModel data refresh keyed on Unit:

## Goals / Non-Goals

- Goal: deliver `Key session-scoped LaunchedEffect refreshes on the session id, not Unit` with observable evidence.
...
- Treat this OpenSpec change as the normative behavior delta for the `ui` area.
