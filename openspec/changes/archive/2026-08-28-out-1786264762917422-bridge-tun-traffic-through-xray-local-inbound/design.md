...
Portfolio task `OUT-1786264762917422` owns this change. Route Android VPN TUN traffic through Xray's local inbound for the first Xray tunneled outbound profile milestone

## Goals / Non-Goals

- Goal: deliver `Bridge TUN traffic through Xray local inbound` with observable evidence.
