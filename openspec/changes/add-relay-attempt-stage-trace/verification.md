...
| REQ-DGN-1786592449526581-007 | DGN-1786597438110594 | Archive RED/GREEN coverage proves deterministic `effective-config-v1:<sha256>` export while the raw signature and its canary value remain absent; a privacy regression proves custom SNI and QUIC host changes do not affect hash material. Offline analytics RED/GREEN coverage proves strict exported-fingerprint preference with legacy fallback. The full `:core:diagnostics:testDebugUnitTest`, 11-test offline analytics suite, and `./gradlew staticAnalysis` passed. | passed |
| REQ-DGN-1786592449526581-008 | DGN-1786601264405063 | Two archive RED/GREEN regressions prove exact same-session/same-timestamp failure-snapshot correlation and explicit `unavailable` output when only a newer snapshot exists. The projection exports identifier-free VPN/underlay observations, resolver latency, and handover state; an offline analytics RED/GREEN regression proves strict allowlist projection and rejects adjacent identifiers. The full `:core:diagnostics:testDebugUnitTest`, 12-test offline analytics suite, `./gradlew staticAnalysis` (726 tasks), architecture health, task/OpenSpec contracts, JSON/Python syntax checks, and legal review passed. | passed |
