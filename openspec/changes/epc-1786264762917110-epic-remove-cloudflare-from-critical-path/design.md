...
Portfolio task `EPC-1786264762917110` owns this change. Remove Cloudflare from every critical path for Russian users while keeping it as an optional low-priority fallback where it still works

## Goals / Non-Goals

- Goal: deliver `Epic - Remove Cloudflare from critical path` with observable evidence.
