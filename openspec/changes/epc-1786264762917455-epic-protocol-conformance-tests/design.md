...
Portfolio task `EPC-1786264762917455` owns this change. Lock the on-wire correctness of RIPDPI's transport stack with golden-fixture conformance tests, cross-stack chain tests, throughput benchmarks, and protocol-behavior regression tests. These guard the wire contract layer; the completed orchestration-test epic remains available in git history

## Goals / Non-Goals

- Goal: deliver `Epic - Protocol conformance and regression tests` with observable evidence.
