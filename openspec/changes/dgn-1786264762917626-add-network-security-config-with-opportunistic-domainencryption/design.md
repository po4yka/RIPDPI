...
Portfolio task `DGN-1786264762917626` owns this change. Add res/xml/networksecurityconfig.xml with <domainEncryption mode="opportunistic"/> as the base config, and point AndroidManifest.xml at it. Opportunistic unlocks platform ECH when both the library and DNS say yes

## Goals / Non-Goals

- Goal: deliver `Add network-security-config with opportunistic domainEncryption` with observable evidence.
