...
commit_sha: 67c25230768c989453313e9a7976752212f1fc6d
local: passed
local_evidence: 338 native tests, 2148 service/engine JVM tests, 103 Python tests, Clippy, instrumentation compilation, staticAnalysis, architecture, task contracts, arm64 native build and ELF checks passed; privileged/device tests remain separate.
remote_ci: required
remote_ci_evidence: Awaiting automatic CI on the new remote source revision; historical CI does not validate this change.
device: blocked
device_evidence: Pixel 8 Pro appeared during final checks on 2026-08-27 (kernel 6.1, API 37). Source-bound runs need an authorized routed dual-stack fixture and APK/VPN test permission; a kernel <5.7 device is still missing. No physical run was performed.
...
| REQ-VPN-1786264762917166-004 | VPN-1786264762917653 | v4 proc observer and strict evidence mutation tests implemented; fixture and physical-run permission pending | blocked |
...
- `python3 -m unittest scripts.tests.test_android_so_bind_physical_evidence scripts.tests.test_so_bindtodevice_evidence scripts.tests.test_so_bindtodevice_lane`: 103 passed. `shellcheck -x` and `bash -n` passed for the physical runner. Its build command has a RED-to-GREEN regression for the gate-injected Cargo jobs variable: it preserves the gate marker, removes the rejected ambient override, and supplies a two-job native budget with one ABI worker.
...
- Both `:app:assembleGithubFullDebug` and `:app:assembleGithubFullDebugAndroidTest`, together with `staticAnalysis`, passed using the corrected physical-runner build environment, arm64-v8a, native builds enabled, two native jobs and one ABI worker. Both APK ZIP integrity checks passed. These are local debug build results; no APK was installed or exercised on the attached device.
