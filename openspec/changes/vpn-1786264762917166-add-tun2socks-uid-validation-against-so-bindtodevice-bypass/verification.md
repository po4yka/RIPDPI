...
- `cargo nextest run --locked --jobs 3 -p ripdpi-tunnel-core -p ripdpi-flow-app-attribution --all-features --no-fail-fast`: 338 passed; one pre-existing privileged TUN test remains skipped on this macOS host. `cargo clippy` with both crates, all targets/features, and `-D warnings` passed.
