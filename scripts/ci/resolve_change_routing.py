...
def is_documentation_only(paths: list[str], *, force_full: bool) -> bool:
    return not force_full and bool(paths) and all(is_documentation_path(path) for path in paths)
...
def main() -> int:
    ...
    docs_only = is_documentation_only(paths, force_full=args.force_full)
