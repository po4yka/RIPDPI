...
class ReleaseP0ContractsTest(unittest.TestCase):
    def test_candidate_is_built_once_inside_signing_environment(self) -> None:
        ...
        signing_preflight = job(source, "release-signing-continuity")
        producer = job(source, "pluggable-transport-assets")
        self.assertIn("- pluggable-transport-assets", signing)
        self.assertIn("- release-signing-continuity", signing)
        ...
        self.assertIn("environment: release-signing", signing_preflight)
        self.assertIn("timeout-minutes: 5", signing_preflight)
        self.assertIn("Release signing continuity mismatch", signing_preflight)
        self.assertIn("trap cleanup EXIT", signing_preflight)
        ...
        self.assertIn("APK signer continuity mismatch", signing)
        self.assertIn("AAB signer continuity mismatch", signing)
