...
class OnboardingViewModelTest {
    ...
    @Test
    fun `setCurrentPage updates current page`() = runTest {
        val vm = createViewModel()
        vm.uiState.test {
            awaitItem()
            vm.setCurrentPage(2)
    ...
    @Test
    fun `setCurrentPage clamps to valid range`() = runTest {
        val vm = createViewModel()
        vm.uiState.test {
            awaitItem()
    ...
    @Test
    fun `nextPage does not exceed last page`() = runTest {
        val vm = createViewModel()
        vm.setCurrentPage(2)
        vm.uiState.test {
            assertEquals(2, awaitItem().currentPage)
    ...
    @Test
    fun `previousPage decrements and clamps at zero`() = runTest {
        val vm = createViewModel()
        vm.setCurrentPage(1)
        vm.uiState.test {
            assertEquals(1, awaitItem().currentPage)
