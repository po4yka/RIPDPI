...
class TestProcessDnsProbeServiceHandle internal constructor(
    ...
    fun awaitNonVpnDefaultNetwork(timeoutMs: Long): Boolean {
        require(timeoutMs in 1..Int.MAX_VALUE.toLong()) { "Invalid non-VPN default-network timeout: $timeoutMs" }
        check(!closed.get()) { "Test-process network probe service is closed" }
        val data = Parcel.obtain()
        val reply = Parcel.obtain()
        try {
            data.writeInterfaceToken(TestNetworkProbeServiceDescriptor)
            data.writeInt(timeoutMs.toInt())
            check(binder.transact(TestNetworkProbeAwaitNonVpnTransactionCode, data, reply, 0)) {
                "Test-process network probe service rejected the non-VPN await transaction"
            }
            reply.readException()
            return reply.readInt() == 1
        } finally {
