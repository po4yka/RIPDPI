...
// SSL_set_client_random pre-seeds the ClientHello random field with |random|.
// |random_len| must equal |SSL3_RANDOM_SIZE| (32). Must be called before
// |SSL_connect| or |SSL_do_handshake|. Returns one on success, zero on error.
// The pre-seeded value is used exactly once; subsequent handshakes on the same
// SSL object generate a fresh random normally.
