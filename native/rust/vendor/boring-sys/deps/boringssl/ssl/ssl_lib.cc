...
int SSL_set_client_random(SSL *ssl, const uint8_t *random, size_t random_len) {
  if (random_len != SSL3_RANDOM_SIZE) {
    return 0;
  }
  OPENSSL_memcpy(ssl->s3->client_random, random, SSL3_RANDOM_SIZE);
  ssl->s3->client_random_overridden = true;
  return 1;
}
