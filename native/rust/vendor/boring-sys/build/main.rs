...
fn build_boringssl_or_get_prebuilt(config: &Config) -> &Path {
    ...
    BUILD_SOURCE_PATH.get_or_init(|| {
        ...
        // Building `ssl` also produces `libcrypto.a`; calling `build()` a second time
        // on the same cmake::Config triggers an unnecessary reconfigure that breaks
        // Android cross-compilation on macOS.
        let path = cfg.build_target("ssl").build();
...
fn emit_link_directives(config: &Config) {
    ...
    if let Some(cpp_lib) = get_cpp_runtime_lib(config) {
        // Keep the C++ runtime after the BoringSSL static archives. Static link
        // order matters for Android NDK libc++/libc++abi symbols.
