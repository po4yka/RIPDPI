...
pub fn classify_outbound_payload(config: &RuntimeConfig, payload: &[u8]) -> OutboundPayloadInfo {
    OutboundPayloadInfo {
        ...
        is_tls: ripdpi_runtime_services::decision_helpers::is_tls_client_hello_payload(payload),
    }
}

pub fn extract_payload_host(config: &RuntimeConfig, payload: &[u8]) -> Option<String> {
    ripdpi_runtime_services::decision_helpers::extract_host(config, payload)
}
...
pub fn classify_udp_payload(config: &RuntimeConfig, payload: &[u8]) -> UdpPayloadInfo {
    let host_info = ripdpi_runtime_services::decision_helpers::extract_host_info(config, payload);
    UdpPayloadInfo {
