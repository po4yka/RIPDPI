...
/// A `vless_reality` profile carrying the default / explicit `xtls-rprx-vision`
/// flow must NOT advertise Vision addons once its transport is xHTTP: Vision is
/// invalid over an HTTP/2 tunnel and xray-core rejects a non-empty flow on a
/// non-raw transport. The builder clears the inner flow for the xHTTP sub-mode,
/// and the loopback fixture rejects any non-empty addons block on the wire — so
/// this round-trip only succeeds because the flow was stripped. Without the
/// clear it fails at the fixture's addons-length guard.
#[tokio::test]
async fn cross_stack_vless_over_xhttp_strips_vision_flow() {
    let server = XhttpRealityLoopback::start().await.expect("start xhttp/reality fixture");
    ...
    let backend = build_backend(&config).await.expect("build vision-flagged VLESS-over-xHTTP backend");
    let target = RelayTargetAddr::Ip(SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), server.target_port()));
    let mut stream =
        backend.connect_tcp(&target).await.expect("connect must succeed because Vision flow is cleared for xHTTP");

    let payload = b"vision flow must be stripped over xhttp".as_slice();
    stream.write_all(payload).await.expect("write cross-stack payload");
    let mut echoed = vec![0_u8; payload.len()];
    stream.read_exact(&mut echoed).await.expect("read cross-stack payload");
    assert_eq!(echoed, payload, "xHTTP round-trip must echo once the Vision flow is stripped");
}
