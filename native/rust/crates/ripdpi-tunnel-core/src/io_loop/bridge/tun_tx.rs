...
pub(in crate::io_loop) async fn flush_device_tx_queue(
    ...
    while let Some(pkt) = device.tx_queue.pop_front() {
        ...
        // emit before `try_send` because the observer captures
        // intent-to-write; a WouldBlock retry below still sends the SAME
        // packet exactly once on success. No `.await` between observer
