...
pub fn get_observed_ttl(stream: &std::net::TcpStream) -> Option<u8> {
    ...
    let ret = unsafe { libc::getsockopt(fd, libc::IPPROTO_IP, libc::IP_TTL, (&mut ttl as *mut _).cast(), &mut len) };
...
pub fn get_observed_ttl(_stream: &std::net::TcpStream) -> Option<u8> {
    None
}
