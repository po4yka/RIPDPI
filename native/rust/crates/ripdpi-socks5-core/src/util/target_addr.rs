...
pub async fn read_address<T: AsyncRead + Unpin>(stream: &mut T, atyp: u8) -> Result<TargetAddr, AddrError> {
    let addr = match atyp {
        ...
        consts::SOCKS5_ADDR_TYPE_DOMAIN_NAME => {
            ...
            let len = read_exact!(stream, [0]).map_err(|err| AddrError::DomainLenUnreadable(err))?[0];
            let domain =
                read_exact!(stream, vec![0u8; len as usize]).map_err(|err| AddrError::DomainContentUnreadable(err))?;
    ...
    let addr: TargetAddr = match addr {
        Addr::V4([a, b, c, d]) => {
            (Ipv4Addr::new(a, b, c, d), port).to_target_addr().map_err(|err| AddrError::AddrConversionFailed(err))?
        }
        Addr::V6(x) => {
            (Ipv6Addr::from(x), port).to_target_addr().map_err(|err| AddrError::AddrConversionFailed(err))?
        }
