...
pub enum ReplyError {
    ...
    #[error("Unknown SOCKS5 reply code 0x{0:02x}")]
    OtherReply(u8),
}
...
pub fn new_udp_header<T: ToTargetAddr>(target_addr: T) -> Result<Vec<u8>, UdpHeaderError> {
    ...
    header.append(&mut target_addr.to_target_addr().map_err(UdpHeaderError::ToTargetAddr)?.to_be_bytes()?);
...
mod test {
    ...
    fn mock_udp_assosiate_no_auth() {
        ...
        block_on(async {
            ...
            let tunnel = client::Socks5Datagram::bind(backing_socket, "[::]:0").await.unwrap();
            let mock_udp_server = UdpSocket::bind("[::1]:0").await.unwrap();
            let mock_address = mock_udp_server.local_addr().unwrap();

            tunnel.send_to(b"hello world!", mock_address).await.unwrap();
            ...
            mock_udp_server.send_to(b"hello world!", addr).await.unwrap();
