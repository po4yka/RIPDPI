...
`ripdpi-dns-resolver` owns RIPDPI's native encrypted DNS client paths. Current implemented modes are DoH, DoT, DNSCrypt, DoQ, and ODoH client mode.

## ODoH Client Mode Constraints

ODoH support is client-only and follows RFC 9230: the client encrypts DNS wire queries to an Oblivious Target with HPKE, sends the encrypted ODoH message through an Oblivious Proxy over HTTPS, and decrypts the target response returned through that proxy.
