...
fn upstream_silent_drop_fault_is_classified_end_to_end() {
    ...
    config.groups[0].matches.detect = ripdpi_proxy_runtime_adapter::model::config::DETECT_SILENT_DROP
        | ripdpi_proxy_runtime_adapter::model::config::DETECT_TCP_RESET;
...
fn tls_round_trip_with_split_desync_completes_handshake() {
    ...
    config.groups[0].actions.tcp_chain.push(TcpChainStep::new(
        ...
        ripdpi_proxy_runtime_adapter::model::config::OffsetExpr::absolute(5),
    ));
...
fn delay_connect_uses_plain_fallback_when_split_group_payload_does_not_match() {
    ...
    config.groups[0].actions.tcp_chain.push(TcpChainStep::new(
        TcpChainStepKind::Split,
        ripdpi_proxy_runtime_adapter::model::config::OffsetExpr::absolute(1),
    ));
    config.network.delay_conn = true;

    let mut fallback = DesyncGroup::new(1);
    fallback.matches.detect = ripdpi_proxy_runtime_adapter::model::config::DETECT_CONNECT;
...
fn route_advancement_fires_on_tcp_reset_and_second_group_handles_traffic() {
    ...
    let mut config = ephemeral_proxy_config(&["--ip", "127.0.0.1"]);
    config.groups[0].matches.detect = ripdpi_proxy_runtime_adapter::model::config::DETECT_TCP_RESET
        | ripdpi_proxy_runtime_adapter::model::config::DETECT_SILENT_DROP;
    config.timeouts.timeout_ms = 500;
