...
pub(super) use ripdpi_proxy_runtime_adapter::model::session::{
    FirstOutboundPayloadPolicy, OutboundPayloadInfo, ParsedSocks5UdpPacket, PayloadHostExtractor, ProxyReply, S_ATP_I4,
    S_ATP_I6, S_AUTH_BAD, S_AUTH_NONE, S_AUTH_USERPASS, S_ER_CMD, S_ER_CONN, S_ER_GEN, S_ER_HOST, S_ER_NET, S_ER_TTL,
    S_VER5, SocketType, UdpPacketParser, UdpPayloadClassifier, UdpPayloadInfo, encode_http_connect_reply,
    ...
    parse_socks5_request, read_upstream_socks_reply,
};

use ripdpi_proxy_runtime_adapter::model::config::RuntimeConfig;
#[cfg(test)]
use ripdpi_proxy_runtime_adapter::model::session::parse_socks5_udp_packet_with;
use ripdpi_proxy_runtime_adapter::model::session::{
    classify_udp_payload_with, first_outbound_payload_policy, parse_socks5_udp_packet_with_host_from_parser,
    payload_host_extractor, udp_packet_parser, udp_payload_classifier,
};
...
pub(in crate::runtime) fn runtime_parse_socks5_udp_packet_with_host<'a>(
    parser: &UdpPacketParser,
    packet: &'a [u8],
    resolve_name: impl FnMut(&str, SocketType) -> Option<SocketAddr>,
) -> Option<ParsedSocks5UdpPacket<'a>> {
    parse_socks5_udp_packet_with_host_from_parser(parser, packet, resolve_name)
}
