...
use ripdpi_runtime_policy::runtime_policy::{ConnectionRoute, RetrySelectionPenalty, TransportProtocol};
...
pub(super) fn note_retry_success(
    ...
    state.adaptive.note_retry_success(&state.config, target, group_index, host, payload, transport)
}
...
pub(super) fn note_retry_failure(
    ...
    state.adaptive.note_retry_failure(&state.config, target, group_index, host, payload, transport, now_millis())
}
...
pub(super) fn build_retry_selection_penalties(
    ...
    state.adaptive.build_retry_penalties(&state.config, target, host, payload, transport, now_millis())
}
...
pub(super) fn apply_retry_pacing_before_connect(
    ...
    state.adaptive.apply_retry_pacing(
        &state.config,
        target,
        route.group_index,
        host,
