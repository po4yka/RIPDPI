...
pub(crate) fn validate_tcp_chain(steps: &[TcpChainStep]) -> Result<(), ProxyConfigError> {
    ...
    for (index, step) in steps.iter().enumerate() {
        ...
        if kind.is_tls_prelude() {
            ...
            saw_send_step = true;
            if kind == TcpChainStepKind::SeqOverlap {
                if saw_seqovl {
            ...
            if kind == TcpChainStepKind::MultiDisorder {
            ...
            if kind == TcpChainStepKind::IpFrag2 {
                saw_ipfrag2 = true;
        ...
        if matches!(kind, TcpChainStepKind::FakeSplit | TcpChainStepKind::FakeDisorder) && index + 1 != steps.len() {
            return Err(ProxyConfigError::InvalidConfig(format!(
