...
// ---------------------------------------------------------------------------
// SOCKS5 helpers (inlined -- bench crate cannot import test support modules)
// ---------------------------------------------------------------------------
...
fn socks5_connect(proxy_port: u16, dst_port: u16) -> TcpStream {
    ...
    // Read reply header (4 bytes) + variable tail
...
impl BenchInfra {
    fn start() -> Self {
        ...
        config.network.listen.listen_port = 0; // ephemeral port
...
impl Drop for BenchInfra {
    fn drop(&mut self) {
        ...
        // Wake the proxy accept loop by connecting to its port.
...
fn bench_relay_throughput(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("relay/tcp-echo");

    let cases: &[(&str, usize, usize)] = &[("1MiB", 1024 * 1024, 1), ("64KiB-x16", 64 * 1024, 16), ("1KiB", 1024, 1)];

    for &(label, chunk_size, chunk_count) in cases {
        let total_bytes = chunk_size * chunk_count;
        let payload = vec![0xAB_u8; chunk_size];

        group.throughput(Throughput::Bytes(total_bytes as u64));
        group.bench_with_input(
            BenchmarkId::new("relay", label),
            &(chunk_size, chunk_count),
            |b, &(_chunk_size, chunk_count)| {
                b.iter(|| {
                    let mut stream = socks5_connect(infra.proxy_port, infra.echo_port);
                    for _ in 0..chunk_count {
                        stream.write_all(&payload).expect("write payload");
                    }
                    // Shut down the write half so the echo server sees EOF and
                    // finishes echoing all received bytes.
                    stream.shutdown(std::net::Shutdown::Write).expect("shutdown write");
                    let mut received = vec![0u8; total_bytes];
                    stream.read_exact(&mut received).expect("read echo");
...
criterion_group! {
    ...
    targets = bench_relay_throughput
}

criterion_main!(relay_throughput);
