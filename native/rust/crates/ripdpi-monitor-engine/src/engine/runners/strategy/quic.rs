...
use super::tcp::{execution_should_stop, sleep_within_active_deadline};
...
impl ExecutionStageRunner for StrategyQuicRunner {
    ...
    fn run(
        ...
        ripdpi_diagnostics_contracts::util::with_scan_io_deadline(deadline, || {
            ...
            let Some(prepared) = selection::prepare_quic_candidates(plan, runtime, fake_ttl_available, ipfrag_caps)
            else {
                return RunnerOutcome::Completed;
            };
            let selection::PreparedQuicCandidates { mut pending_quic_specs, quic_targets, quic_candidate_total } =
                prepared;
            ...
            while !pending_quic_specs.is_empty() {
                ...
                {
                    tracing::warn!("strategy probe: QUIC suite deadline-terminated during inter-candidate pause");
                    break;
                }
            }
