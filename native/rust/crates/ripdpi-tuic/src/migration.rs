...
pub(crate) struct QuicMigrationState {
    ...
    pub(crate) previous_socket: Option<std::net::UdpSocket>,
}

impl ClientInner {
    pub(crate) async fn begin_quic_migration(&self) -> io::Result<bool> {
        let should_attempt = {
    ...
    pub(crate) async fn rollback_quic_migration(&self, reason: &str) -> io::Result<()> {
        let previous_socket = {
            let mut state = self.migration.lock().await;
        ...
        self.endpoint.rebind(previous_socket)?;
