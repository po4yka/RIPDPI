...
use super::registry::{ProxySessionState, control_for_proxy_stop, ensure_proxy_destroyable, try_mark_proxy_running};
...
fn loom_two_concurrent_starts_one_wins() {
    loom::model(|| {
        ...
        let t1 = loom::thread::spawn(move || {
            ...
            try_mark_proxy_running(&mut s, ctrl1).is_ok()
        });

        let state2 = state.clone();
        let t2 = loom::thread::spawn(move || {
            let mut s = state2.lock().unwrap();
            try_mark_proxy_running(&mut s, ctrl2).is_ok()
        });
...
fn loom_concurrent_start_and_stop() {
    loom::model(|| {
        ...
        let t_start = loom::thread::spawn(move || {
            let mut s = state1.lock().unwrap();
            let _ = try_mark_proxy_running(&mut s, ctrl1);
        });

        let state2 = state.clone();
        let t_stop = loom::thread::spawn(move || {
            let s = state2.lock().unwrap();
            control_for_proxy_stop(&s).ok()
        });
...
fn loom_stop_then_destroy_consistent() {
    loom::model(|| {
        let ctrl = std::sync::Arc::new(EmbeddedProxyControl::default());
        let state = Arc::new(Mutex::new(ProxySessionState::Running { control: ctrl }));

        let state1 = state.clone();
        let t_stop = loom::thread::spawn(move || {
            let mut s = state1.lock().unwrap();
            if control_for_proxy_stop(&s).is_ok() {
