...
- Remaining CONNECT-UDP conformance gaps: there is no H3 CONNECT-UDP server fixture proving request-stream DATAGRAM association and full UDP round-trip, no bearer/mTLS generic-provider E2E against a conformant proxy, and no cross-implementation provider test. QUIC-varint, Capsule framing, response validation, Proxy-Status mapping, and H2 CONNECT-UDP fallback have deterministic local coverage.
