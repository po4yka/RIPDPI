...
**Responsibility:** the MASQUE proxy transport — HTTP/3 (with HTTP/2 fallback) CONNECT-UDP, standard generic/self-hosted provider authentication, and RFC conformance tests.
**Layer:** L7 — relay transports.

## Stable identifiers / contracts

Selected by `relay_kind = "masque"`. The MASQUE / CONNECT-UDP exchange is governed by RFC 9298 and RFC 9297. HTTP/3 to HTTP/2 fallback is reported in telemetry; provider auth failures are classified by cause.
...
- Commercial-relay provider adapters such as iCloud Private Relay and Cloudflare proprietary auth.
