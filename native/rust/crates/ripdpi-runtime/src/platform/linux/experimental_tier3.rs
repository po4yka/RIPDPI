...
use crate::platform::experimental_tier3::{
...
pub(crate) fn send_syn_hide_tcp(spec: &SynHideTcpSpec, protect_path: Option<&str>) -> io::Result<()> {
    let packet = build_syn_hide_tcp_packet(spec)?;
    send_ip_packet(spec.target, &packet, protect_path)
}

pub(crate) fn send_icmp_wrapped_udp(spec: &IcmpWrappedUdpSpec, protect_path: Option<&str>) -> io::Result<()> {
...
pub(crate) fn recv_icmp_wrapped_udp(
...
fn send_ip_packet(target: SocketAddr, packet: &[u8], protect_path: Option<&str>) -> io::Result<()> {
    let socket = match target {
        SocketAddr::V4(_) => {
            ...
            super::super::protect_socket(&socket, protect_path)?;
