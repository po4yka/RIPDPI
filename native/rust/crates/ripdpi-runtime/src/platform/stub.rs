...
pub fn wait_tcp_stage(_stream: &TcpStream, _wait_send: bool, _await_interval: std::time::Duration) -> io::Result<()> {
    Ok(())
}
...
mod tests {
    ...
    fn unsupported_operations_report_unsupported() {
        ...
        assert_eq!(
            original_dst(&client).expect_err("original dst must be unsupported").kind(),
            io::ErrorKind::Unsupported
        );
    }
