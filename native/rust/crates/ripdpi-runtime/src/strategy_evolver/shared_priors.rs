//! Static shared-priors loader for the offline learner (P4.4.4, ADR-011).
//!
//! Parses a newline-delimited JSON stream of Beta posteriors that can be
//! shipped as a static asset (bundled or GitHub-hosted) and consumed by
//! [`crate::strategy_evolver::thompson_sampling`] as a prior boost. Each
//! line is one record:
//!
//! ```json
...
//! - Combo identity is a u64 hash, computed off the combo wire form so the
//!   priors can travel without leaking the combo definition itself.
...
//! - Network fetch / refresh scheduling (no backend; would be GitHub-hosted).
//! - Signed-manifest verification (depends on a release-signing infra not
//!   yet stood up).
//! - Compression (`MAX_RAW_PAYLOAD` is enforced on the uncompressed input).
//!
//! Those pieces remain "future work" per the ADR; this module exists so the
//! format and validator are pinned now and can be wired into an actual
//! delivery channel without redesigning the interchange.
