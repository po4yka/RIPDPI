...
mod tests {
    ...
    fn runtime_state(config: RuntimeConfig, telemetry: Option<StdArc<dyn RuntimeTelemetrySink>>) -> RuntimeState {
        RuntimeState {
            ...
            retry_stealth: crate::sync::Arc::new(crate::sync::Mutex::new(RetryPacer::default())),
            strategy_evolver: crate::sync::Arc::new(crate::sync::Mutex::new(StrategyEvolver::new(false, 0.0))),
