...
description: Audits `async fn` cancel-safety across every `.await` point in a diff and requires a `# Cancel safety:` rustdoc block. Use when adding or changing async code, or for periodic async-safety audits.
