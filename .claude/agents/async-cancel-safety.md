...
5. **For each guard held across `.await`:**
   - `std::sync::Mutex`, `parking_lot::Mutex`, `tokio::sync::MutexGuard`, `sqlx::Transaction`, `mpsc::Permit`, file `OwnedFd` — flag every one. Use `cargo clippy -- -W clippy::await_holding_lock -W clippy::await_holding_invalid_type` for the lints; cross-check manually for types not yet in the disallowed list.
...
io_loop.rs:122  tick_phase_6   cancel-safe          all 5 select arms call cancel-safe primitives (tun.readable, sleep, mpsc::recv, cancel.cancelled)
session.rs:88   commit_record  NOT cancel-safe      db.insert().await followed by send_ack().await — partial completion possible
```
