...
RIPDPI-specific: AGENTS.md "Flow hot-path discipline" already calls for `WhileSubscribed(5_000)` over `Eagerly` to prevent leaks across config changes — but only when consumers don't synchronously read `.value`. Audit each `stateIn` site for which constraint applies.
...
- `references/side-effects.md` — collecting event flows and wiring side effects in Compose.
- `references/state-holder-vs-viewmodel.md` — where state holders expose flows to UI.
