...
This keeps screens previewable, testable, and easier to reuse across surfaces.
...
If that UI-local state grows into coordinated behavior with multiple related fields and operations, use `references/state-hoisting.md` to decide whether it should become a plain state holder class remembered in composition.
...
- Prefer explicit lambdas (`onRetryClick`, `onItemSelected`) over passing a whole ViewModel/component.
...
- Frame-rate or UI-local values that should not force whole-tree recomposition when they change: prefer provider lambdas and deferred reads per `references/deferred-reads.md`.

## Side effects

`references/side-effects.md` covers effect APIs (`LaunchedEffect`, `DisposableEffect`, `SideEffect`), keys, cleanup, and `rememberUpdatedState`.
...
If effect handling grows, extract `ProfileEffects(viewModel, snackbarHostState)` rather than pushing the ViewModel into the UI composable.
...
| Child composables take `viewModel`/`component` | Dependencies leak through the tree | Pass only the state/callbacks that child needs |
...
- `references/state-hoisting.md` — deciding where UI element state and UI logic should live, including plain state holder classes.
- `references/deferred-reads.md` — provider lambdas for frame-rate values that cross composable boundaries.
- `references/side-effects.md` — choosing effect APIs and where to wire them.
