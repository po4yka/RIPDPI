...
- `indexOf()` / `lastIndexOf()` / `indexOfFirst { }` called inside a `LazyListScope` item factory -- O(n^2) scroll cost and crash risk when identity moves; use `itemsIndexed` instead -> [lists](https://developer.android.com/develop/ui/compose/lists)
...
**RIPDPI note:** This project uses explicit `@Immutable`/`@Stable` on all UI model classes and maintains `app/compose-stability.conf`. Expect higher-than-average `skippable%`. Verify continued coverage rather than introducing stability annotations from scratch.
...
- hardcoded magic numbers like `.padding(12.dp)` or `.size(24.sp)` or explicit `Color(0xFF...)` instead of routing through `RipDpiThemeTokens` or `MaterialTheme` tokens. A clean theme is vital for dark mode and accessibility -> [theming](https://developer.android.com/develop/ui/compose/designsystems/material3)
