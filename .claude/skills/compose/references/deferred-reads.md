...
RIPDPI hotspots flagged in AGENTS.md / the `compose-performance` quick-wins list — HomeScreen infinite-transition animation, AdvancedSettingsScreen recomposition scope, LazyColumn filter screens (LogsScreen, DiagnosticsScreen, HistoryScreen) — are all candidates for the patterns below.
...
- `references/state-holder-vs-viewmodel.md` — where state-holder vs plain UI split applies when passing providers/lambdas across boundaries.
- `references/performance.md` — parameter stability and compiler reports.
- `references/modifiers.md` — child composables need a normal `modifier` parameter before callers can move visual reads into modifiers.
