...
**Compose module**: `:app` is the only module with the `ripdpi.android.compose` convention plugin. Scope all `@Composable`, `remember`, `LaunchedEffect`, and UI-related searches to `app/src/main/kotlin/`.

**Key directories**:
- `app/src/main/kotlin/com/poyka/ripdpi/ui/screens/` -- per-feature screen composables
- `app/src/main/kotlin/com/poyka/ripdpi/ui/components/` -- design system components (buttons, cards, inputs, indicators, feedback, navigation, scaffold)
- `app/src/main/kotlin/com/poyka/ripdpi/ui/theme/` -- RipDpiTheme, Color, Type, Spacing, Shape, Motion
- `app/src/main/kotlin/com/poyka/ripdpi/ui/navigation/` -- NavHost, BottomNavBar, Route
- `app/src/main/kotlin/com/poyka/ripdpi/activities/` -- ViewModels + UI models (DiagnosticsUiModels, SettingsUiModels, HistoryUiModels)

**Non-Compose modules** (do NOT search for Compose UI definitions):
- `:core:data` -- Protobuf + DataStore settings
- `:core:diagnostics` -- active/passive diagnostics logic
- `:core:engine` -- Rust JNI bridge
- `:core:service` -- VPN/proxy foreground services
- `:core:detection` -- detection check module
...
For RIPDPI, the key paths are listed in the module scope section above.
...
- `indexOf(...)` / `lastIndexOf(...)` / `indexOfFirst { ... }` called inside a `LazyListScope` item factory -- O(n^2) scrolling cost and crash risk if identity moves; prefer `itemsIndexed`
...
Focus on shared components in `ui/components/` and internal UI kit code, not every screen.

### Search For

- shared component directories: `ui/components/`
