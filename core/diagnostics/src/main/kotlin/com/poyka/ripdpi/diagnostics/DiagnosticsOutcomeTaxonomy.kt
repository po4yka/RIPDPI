...
object DiagnosticsOutcomeTaxonomy {
    ...
    private fun bucketForProbeOutcome(
        ...
        when (probeType) {
            ...
            "dns_integrity" ->
                when (outcome) {
                    "dns_match" -> DiagnosticsOutcomeBucket.Healthy
                    "dns_expected_mismatch" -> DiagnosticsOutcomeBucket.Attention
                    "udp_blocked" ->
                        if (pathMode == ScanPathMode.RAW_PATH) DiagnosticsOutcomeBucket.Attention else DiagnosticsOutcomeBucket.Inconclusive
                    "udp_skipped_or_blocked" ->
                        if (pathMode == ScanPathMode.IN_PATH) DiagnosticsOutcomeBucket.Attention else DiagnosticsOutcomeBucket.Inconclusive
                    "dns_substitution", "encrypted_dns_blocked", "dns_unavailable" -> DiagnosticsOutcomeBucket.Failed
                    else -> legacyBucketForOutcome(outcome)
                }
            ...
            "domain_reachability" ->
                when (outcome) {
                    ...
                    "tls_version_split", "http_ok" -> DiagnosticsOutcomeBucket.Attention
            ...
            "strategy_https" ->
                when (outcome) {
                    "tls_ok" -> DiagnosticsOutcomeBucket.Healthy
                    "tls_version_split" -> DiagnosticsOutcomeBucket.Attention
