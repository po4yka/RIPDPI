 ...
 *   upstream socket.  ECH and Cronet are NOT available on this path.
 *
 * - [OwnedStack]: traffic originates from our own browser/SDK components
 *   (Cronet + ECH).  VpnService.protect() is not used; the SDK controls the
 *   full network stack.
 *
...
sealed class TransportMode {
    ...
    /** Cronet + ECH path — handles traffic from our own SDK only. */
