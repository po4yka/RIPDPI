...
class AndroidNetworkMetadataProvider
    ...
    private fun resolveWifiDetails(capabilities: NetworkCapabilities?): WifiNetworkDetails? {
        ...
        return WifiNetworkDetails(
            ...
            rxLinkSpeedMbps = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) wifiInfo?.rxLinkSpeedMbps?.takeIf { it > 0 } else null,
            txLinkSpeedMbps = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) wifiInfo?.txLinkSpeedMbps?.takeIf { it > 0 } else null,
    ...
    private fun resolveCellularDetails(capabilities: NetworkCapabilities?): CellularNetworkDetails? {
        ...
        return CellularNetworkDetails(
            carrierName = sanitizeTelephonyValue(telephony.simCarrierIdName?.toString() ?: telephony.networkOperatorName),
    ...
    private fun invokeBoolean(
        ...
        runCatching { target.javaClass.getMethod(methodName).invoke(target) as? Boolean }.getOrNull()
