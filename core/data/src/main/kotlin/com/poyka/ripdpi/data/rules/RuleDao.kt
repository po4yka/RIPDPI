...
abstract class RuleDao {
    ...
    /** Deletes every routing rule. Used by the atomic backup-restore swap. */
    @Query("DELETE FROM routing_rules")
    abstract suspend fun deleteAll()

    /**
     * Atomically replaces the entire rule table with [rules] in one Room
     * transaction: the existing rows are deleted and [rules] inserted. Either the
     * whole swap commits or none of it does, so a restore never leaves a partially
     * overwritten rule set. Each entity's [RuleEntity.id] should be its default
     * (`0L`) so Room assigns fresh autogenerated ids.
     */
    @Transaction
