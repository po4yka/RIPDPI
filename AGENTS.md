...
RIPDPI is an offline-first Android VPN/proxy application for DPI bypass. Jetpack Compose provides the UI, Android services own VPN/proxy lifecycles, and repository-owned Rust modules implement the native data plane, diagnostics, relays, tunnels, and JNI adapters.
...
Repository tasks live under `docs/tasks/`. Use the `repo-task-board` skill for creating, updating, triaging, or closing them.

- `docs/tasks/issues/<slug>.md` is the source of truth.
- `docs/tasks/board.md` is generated and read-only.
- `docs/tasks/README.md` defines schema and lifecycle.

Shared-file ownership for parallel work must be recorded before writers start.
