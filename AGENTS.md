...
4. Agent runtime permissions for `android` CLI calls: - **Claude Code**: copy `.claude/settings.example.json` to your local (gitignored) `.claude/settings.json` -- it carries `permissions.allow: ["Bash(android:*)"]`, a wildcard covering every subcommand (`android sdk`, `android docs search`, `android journeys`, `android studio`). - **Codex**: approve the project once via `trust_level = "trusted"` in `~/.codex/config.toml` (Codex prompts on first run). Codex has no per-command allowlist; project trust covers all shell invocations including `android docs` / `android sdk` / `android journeys`.
5. **Android CLI 1.0 agent features** (optional, local dev): run `android init` once to install the `android-cli` skill (or `android skills add --all` for the full Android skill library) -- these write to agent skill directories under `$HOME`, never the repo tree. `android journeys` runs the natural-language UI flows under `journeys/` (see `scripts/ci/run-android-journeys-emulator.sh` and the `android-test-runner` agent). The `android studio ...` commands (`studio check`, `studio version-lookup ndk agp gradle`, `studio open-file`) bridge to a running Android Studio (Quail 2 Canary 1+ with Gemini) -- preview, local-only, not wired into CI.
