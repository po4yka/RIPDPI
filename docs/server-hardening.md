...
Required server-side follow-up belongs in the sibling deploy repo task `../ripdpi-vpn-deploy/docs/tasks/issues/asn-exposure-denylist-gate.md`: a design boundary document, placeholder-only schema fixtures, redacted dry-run summaries, disabled-by-default render tests, canary/log-only rollout notes, false-positive monitoring, and rollback criteria. Until those artifacts exist, RIPDPI should only describe the control objective and source provenance; diagnostics copy must call this infrastructure hardening rather than a local DPI bypass strategy.
