...
A31 local validation passed 535 tests across diagnostics transport, runner, TLS, HTTP, DNS and monitor-engine, with five pre-existing ignored tests and zero failures. Full transport coverage includes 59 passing tests. Clippy for transport and runner with all targets and warnings denied passed. Error precedence and deadline exhaustion received independent review. The final exact-SHA hosted CI result is recorded in PR 495 and the task closure evidence.
