...
| 11.9 | Complete. Strategy-pack TLS profile sets now carry explicit acceptance report refs and review dates, and CI validates catalog `allowedProfileIds`, refresh-log metadata, and evidence refs against the shipped template families plus acceptance corpus/report | `python3 -m unittest scripts.tests.test_tls_catalog_refresh`; `python3 scripts/ci/check_tls_catalog_refresh.py`; `:core:data:testDebugUnitTest` strategy-pack catalog tests |
