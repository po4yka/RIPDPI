...
| Initial transport race with primary unavailable | Select a viable transport class and carry application traffic | In a disposable APK, only the configured primary REALITY server was replaced by loopback. The clean-seed startup race selected Hysteria2; no REALITY handshake completed, Android reported a live VPN network, and Chrome from a separate UID loaded the HTTPS control page. No packet-level direct-window capture was available | Partial; transport fallback passed | — |
...
| Brief total network loss while REALITY active | Rebind/restart and resume traffic | Wi-Fi loss and restoration were observed over USB; the live VPN network returned and Chrome from a separate UID loaded a fresh HTTPS control URL after recovery | Pass | — |
...
- Process-death regression coverage verifies durable session recovery, stale-pointer and cancellation handling, serialized start/stop arbitration, accepted notification-stop semantics, and explicit-stop non-resurrection. The following checks passed on the rebased commit series through `cc3969941`:
  - `./gradlew -Pripdpi.skipNativeBuild=true :core:service:testDebugUnitTest :core:data:runtime-state:testDebugUnitTest :app:compileGithubSimpleDebugKotlin`
  - the affected service, runtime-state, and app ktlint/detekt tasks plus `:app:lintGithubSimpleDebug`
  - `python3 scripts/ci/check_architecture_health.py`
  - `cd native/rust && cargo metadata --locked --no-deps --format-version 1`
  - `./gradlew -Pripdpi.localNativeAbis=arm64-v8a :app:assembleGithubSimpleDebug`
- The deterministic unavailable-primary APK was built with real arm64 native libraries. After the test, the Pixel was cleared and reinstalled from the canonical APK; its embedded bundle matched the canonical asset byte-for-byte, VLESS+Reality reconnected, and Chrome HTTPS/DNS passed.
