...
- 2026-05-12: Surfaced ECH readiness in the DPI probe suite/tool UI as an explicit opt-in probe, wired bundled/user override targets into suite execution, capped suite ECH concurrency at 4, and attached `vanillaTlsOk` from same-run domain reachability results when available. ECH is visible but not selected by default until the native handshake bridge exists.
...
- Remaining close gates: native ECH handshake bridge and real ECH validation beyond fake JVM handshakes.
