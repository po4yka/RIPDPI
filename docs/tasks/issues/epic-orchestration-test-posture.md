---
title: Epic - Orchestration test posture
type: epic
status: doing
area: testing
priority: high
owner: unassigned
parent: null
blocks: []
blocked_by: []
created: 2026-04-20
updated: 2026-06-05
---

## Goal

Close the three untested-class gaps the audit surfaced, and build one shared deterministic failure-injection harness that every orchestration- level scenario test reuses. Parser/config fuzzing is already good — this epic is specifically about orchestration and lifecycle, where the bugs hide today.

## Why now

Cache corruption, supervisor lifecycle edges, rollback attempts, and protect-socket stalls all happen rarely in production and are impossible to reproduce without deterministic injection. Every audit-stream fix benefits from having a test that would have caught the original bug, and shares more infrastructure than it would build alone.

## Key decisions

- **One shared harness** (fake clock, scripted network, corrupt-file fixture, scripted exit causes, stall injection for the protect socket) — not bespoke fixtures per scenario.
- **Scenario tests block the fixes they regress-protect** via `blockedBy`, so the harness lands first and the scenarios follow as each matching fix merges.
- **Unit coverage for the three untested classes** (`DefaultStrategyPackService`, `AppStartupInitializer`, `VpnProtectSocketServer`) is a separate task from the scenario harness — different failure mode, different fixture needs.

## Scope

- **In scope:** shared failure-injection harness; unit tests for the three untested classes; scenario tests for cache corruption, repeated startup/shutdown, control-plane rollback, protect-socket stall.
- **Out of scope:** parser/config fuzz coverage (already good); UI tests; end-to-end device tests.

## Ship definition

- [x] Harness documented in the test-module README with a minimal example.
- [x] Four scenario tests use the harness and pass deterministically (no sleep-based waiting).
- [x] Each of the three previously-untested classes has a dedicated test file covering the failure modes the audit called out.
- [x] CI green on main after every fix-and-test pair merges. — Confirmed 2026-06-11: the last *completed* `CI` workflow run on `main` is green (`fe0abb1b1`); every run since is `cancelled` by concurrency (each doc-only push supersedes the prior in-flight run), not failed. The harness + the three untested-class test files predate that green run. Standing invariant — re-confirmed on each future fix-and-test merge.

## Child tasks

**Harness**
- Add orchestration failure-injection harness

**Unit coverage for untested classes**
- Add unit tests for orchestration gaps

**Scenario tests** (each `blockedBy` the harness)
- Add cache-corruption regression test
- Add repeated startup-shutdown supervisor test (closed task)
- Add control-plane rollback attempt test
- Add protect-socket server stall test

Child tasks roll up via the TaskNotes relationships view on this note.

## Dependencies

- Depends on: [[Epic - Control-plane hardening]] (rollback / atomic writes must exist before their regression tests).
- Depends on: [[Epic - Runtime lifecycle and supervisors]] (explicit exit causes must exist before the supervisor lifecycle test).
- Depends on: Epic - Privacy and diagnostics (reworked protect socket must exist before the stall test's assertions make sense).
- Shares test coverage with: [[Epic - Direct-mode diagnostic state machine]] (integration tests per result class, also `blockedBy` this harness).

## Risks / open questions

- Fake-clock discipline: any test that uses real time introduces flakiness. Linter rule to reject real-clock calls inside harness- governed tests?

## Links

- [[ripdpi-android]]
- Child issues: 2

## Work log

- 2026-06-05: Harness exists at `core/service/src/test/.../testsupport/OrchestrationFailureHarness.kt` (ScriptedSupervisorExitSequence, HarnessStallGate, OverlapTracker) with README and minimal examples; all three untested classes have dedicated test files (AppStartupInitializerTest 728 lines, VpnProtectSocketServerTest 360 lines, StrategyPackServiceTest 800 lines); scenario coverage via harness primitives is present across ProxyRuntimeSupervisorTest (repeatedStartStop), VpnProtectSocketServerTest (stall), AssetStrategyPackRepositoryRefreshPolicyTest (cache corruption via CorruptFileFixture, anti-rollback), UpstreamRelaySupervisorTest; CI green state cannot be verified from source.
- 2026-06-05 (epic audit): Re-verified all four artifacts against source; line counts and primitives match exactly. README at `core/service/src/test/kotlin/com/poyka/ripdpi/services/testsupport/README.md` documents the harness with minimal supervisor + stall examples (criterion 1 done). Scenario test methods confirmed: `repeatedStartStopRecoversAfterScriptedCrash` (ProxyRuntimeSupervisorTest), three stall tests (VpnProtectSocketServerTest), corruption + anti-rollback tests (AssetStrategyPackRepositoryRefreshPolicyTest using `CorruptFileFixture`); no `Thread.sleep`/`delay()` in any of the three scenario files and supervisor test uses runTest/advanceUntilIdle/TestServiceClock (criterion 2 done, deterministic). Three untested classes each have dedicated test files (criterion 3 done). CI-green criterion remains unverifiable from source, so it stays unchecked. The named child task files (harness, unit-coverage, four scenario tasks) do not exist as separate `.md` files in this directory — only `spike-censorlab-as-offline-censor-replay-harness.md` carries `parent:` — so rollup rests on epic-level source verification. Status kept at `doing`: 3 of 4 ship criteria verifiably complete, CI-green outstanding/unverifiable.
- 2026-06-11 (CI-green confirmed + last child spike shipped): Ticked ship criterion 4 — CI green on `main` confirmed via `gh run list`: the last *completed* `CI` workflow run is green (`fe0abb1b1`); subsequent runs are `cancelled` by concurrency (doc-only pushes supersede the prior in-flight run), not failures. All four ship criteria are now satisfied; the epic is left `doing` as the standing test-posture home (criterion 4 is an ongoing invariant, and closing/deleting an epic is an owner decision). The last remaining child, `spike-censorlab-as-offline-censor-replay-harness`, is **shipped as a bounded spike with a REJECT-dependency / FORK-ideas decision** and recorded in `test-lab/chaos/tspu/CENSORLAB-EVALUATION.md`; the spike task file is deleted per the done-file convention (decision preserved in the evaluation doc). Evidence summary: CensorLab (arXiv:2412.16349, GPL-3.0, Nix/nightly-Rust, NFQUEUE) is redundant with the in-repo `test-lab/chaos/tspu` NFQUEUE harness (deterministic dry-run green: 53 tests, 63 cells) — adopt rejected; the genuine `tspu` gaps (DoH/DoQ classifier, stateful reassembly, ML-censor emulation) are recorded as prioritised fork-in follow-ups.
