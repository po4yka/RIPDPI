...
- [x] (partial, 2026-05-15) Wire encode/decode paths use the enum (no bare `0x01` / `0x05` literals in encode or decode arms) — **in `ripdpi-vless` only**. `encode_request` writes `ProtocolVersion::V1.wire_byte()`; `parse_request_header` uses `ProtocolVersion::from_wire_byte().is_none()` for rejection.
- [ ] A new `ripdpi-diagnostics-protocols` probe attempts a low-cost handshake and classifies failures into `Reachable`, `VersionMismatch { offered, server_signaled }`, `AuthFailure`, `BlockedOrDropped`, and `Unknown`.
- [ ] `ripdpi-failure-classifier` maps `VersionMismatch` to a distinct user-visible class with a remediation hint ("server upgraded; update client or change profile").
- [ ] Unit tests cover encode/decode of every enum variant plus the classifier outputs for synthetic failure traces.
