...
- Added rooted-device runtime wiring for Lua-originated `rawsend` execution. Raw packet sends now flow through `ripdpi-runtime-platform::experimental::send_raw_ip_packet()`, which tries the registered root-helper socket before direct raw-socket fallback; the root-helper protocol now includes `send_raw_ip_packet` with target address plus raw packet bytes. Verification: `CARGO_TARGET_DIR=/Users/po4yka/GitRep/.codex-targets/ripdpi-root-helper-raw cargo test --manifest-path native/rust/Cargo.toml -p ripdpi-root-helper --locked send_raw_ip_packet -- --nocapture`; `CARGO_TARGET_DIR=/Users/po4yka/GitRep/.codex-targets/ripdpi-root-helper-raw cargo test --manifest-path native/rust/Cargo.toml -p ripdpi-runtime-platform -p ripdpi-root-helper-protocol -p ripdpi-tunnel-android --locked root_helper -- --nocapture`. Still blocked until live rooted-device Lua/rawsend egress is proven.
