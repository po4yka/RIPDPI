...
- Added rooted-device runtime wiring for `udplen` raw packet emission. The TUN injector now calls `ripdpi-runtime-platform::experimental::send_raw_ip_packet()`, allowing the registered root-helper socket to perform privileged raw sends before falling back to direct app-process raw sockets. Verification: `CARGO_TARGET_DIR=/Users/po4yka/GitRep/.codex-targets/ripdpi-root-helper-raw cargo test --manifest-path native/rust/Cargo.toml -p ripdpi-tunnel-core --locked tun_egress_interceptor` passed including `udplen_rule_injects_modified_udp_packet`; `CARGO_TARGET_DIR=/Users/po4yka/GitRep/.codex-targets/ripdpi-root-helper-raw cargo test --manifest-path native/rust/Cargo.toml -p ripdpi-root-helper --locked send_raw_ip_packet -- --nocapture` compiled the root-helper dispatch path. Still blocked until live rooted-device raw UDP egress is proven.
