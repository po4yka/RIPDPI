...
### 2026-08-28 execution ownership and acceptance

- Root writer: `codex/extended-outbound-protocols-20260828` in `/private/tmp/ripdpi-extended-outbound-20260828`; Android editors, service integration, planning, and integration. Root exclusively owns Kotlin/JNI contracts, protobuf, locale sets, capability registries, and dependency manifests/lockfiles unless a specific native seam is delegated before editing.
- Native writer: `codex/extended-outbound-native-20260828` in `/private/tmp/ripdpi-extended-outbound-native-20260828`; SSH/Mieru/AnyTLS Rust implementations, protocol tests, and pinned upstream loopback oracles. No concurrent writes to the root worktree; no production dependency additions without approval.
- Architecture/review agent is read-only. All heavy builds share `build-gate`; previous worktrees and immutable evidence remain intact.
- SSH editor writer: `codex/extended-outbound-ssh-editor-20260828` in `/private/tmp/ripdpi-extended-outbound-ssh-editor-20260828`; SSH editor/route/tests, its locale keys, and the new `NativeRelayVpnActivator` with tests. Root retains the private host-key probe service and Kotlin JNI binding. Native writer owns the new SSH probe JNI export, the existing `ripdpi-ssh` dependency edge and Mieru's existing Tokio feature selection; no new external production dependency is authorized.
- The current completion scope includes all three open children: SSH host-key accept/reject before credentials, Mieru TCP and UDP carriers with mux, and AnyTLS interoperability plus main Mode Editor persistence. Historical deferrals below are not evidence of completion. SSH remains TCP-only; Mieru UDP carrier does not imply UDP-payload support.
- Each protocol needs an independent pinned upstream handshake/payload oracle and bounded stop under load. Editor persistence, service readiness, Android execution, hosted CI, and deployment evidence are recorded separately. Removed protocols and unapproved generic outbound kinds remain excluded.
...
- 2026-08-28: Verified implementation slice: `4d6f848bc` fixes SSH/AnyTLS/Mieru TCP interoperability and owned session teardown; `4fddd82ec` exposes and persists AnyTLS in the main Mode Editor; `d11c4c82c` activates imported Mieru profiles and redacts their credentials; `1fba87185` adds credential-free protected SSH host-key observation. Local evidence: 401 native tests, all nine separately executed pinned upstream cases, 354 Kotlin tests and `staticAnalysis`; none substitutes for Android or hosted CI. Integration candidate is isolated at `codex/extended-outbound-verified-20260828`. The SSH Connect editor remains outside this slice pending review of intentional screenshot changes and explicit fixture blessing. Mieru UDP remains unsupported and open; the SSH EncryptedFile storage criterion is reopened. Epic and children remain `doing`; historical deferral text is not a completion claim.
