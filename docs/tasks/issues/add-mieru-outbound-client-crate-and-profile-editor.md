...
- 2026-08-28: `4d6f848bc` corrects TCP carrier nonce order, authenticated SOCKS CONNECT framing and raw TCP payload handling against pinned upstream commit `155ebbd60f86e472586a60d7ffe58ec8f8682cb1`. Independent tests exchange 64 KiB payloads, cover low/middle/high multiplexing without cross-contamination, and verify Off-mode SOCKS payload and owned carrier shutdown. A full teardown queue aborts the carrier and closes admission instead of silently abandoning close work. `d11c4c82c` makes imported Mieru profiles select the native backend and masks credentials in `toString`. UDP carrier remains unsupported, rejected before DNS/dial, and uncompleted pending the transport security decision; no UDP interoperability claim is made.
