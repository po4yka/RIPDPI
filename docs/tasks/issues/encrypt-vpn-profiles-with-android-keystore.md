...
- [x] #task Encrypt VPN profiles with Android Keystore #repo/RIPDPI #area/vpn #status/done ⏫
...
- [x] Profile and subscription credential blobs are encrypted at rest in internal app storage.
- [x] Android Keystore holds the key-encryption key and uses StrongBox when available and configured.
- [x] Secret wrapper types redact `toString()`, equality debug output, logs, and diagnostics.
- [x] Migration path can import existing plaintext development profiles and remove plaintext copies.
- [x] Tests prove exported diagnostics contain config hashes or fingerprints, not raw credentials.
