...
- 2026-06-05: Re-audit confirms prior finding. `ripdpi-tuic/src/client.rs` has no calls to `classify_failure_payload` or any `TuicFailureKind` construction; `TuicVersionUnsupported` is unreachable at runtime (confirmed via grep of ripdpi-proxy-runtime-adapter, ripdpi-relay-core, and app/). The variant in `response_triggers.rs:104` maps to `0` but is never emitted. Status remains `doing`; no criteria added or removed.
