...
- [ ] #task Add TCP16 FAT-Header DPI Probe with Keep-Alive RST Detection #repo/RIPDPI #area/diagnostics #status/doing ⏫

## Objective

Add `Tcp16FatHeaderProbe` that issues 16 sequential `HEAD` requests over a single keep-alive HTTP/1.1 connection to each of 140 CDN/hosting IPs from `tcp16.json`, padding each request after the first with a 4KB random `X-Pad` header to detect middlebox-induced 16-20KB cumulative-byte RST injection.

## Context

This is the Android port of dpi-detector's Test 4 (`check_tcp_16_20`). Active L7 middleboxes can apply per-flow byte-counting and inject TCP RST when cumulative client→server bytes cross the 16-20KB threshold. The probe detects this by:
...
4. If the middlebox pattern is present, the connection RSTs around request 4-5 (16-20KB cumulative); record `DETECTED at <i*4>KB`
...
- **`hint_rtt` mode**: variant `runWithRttHint(rtt)` skips RTT measurement for a linked SNI-finder workflow that already has the RTT from the initial probe.
...
- Requests 1-15 fail at byte K (16-20 KB) → `DETECTED at K KB` (middlebox-pattern positive)
...
5. **Refactor** — extract OkHttp builder into `Tcp16ProbeClientFactory`; share with the linked SNI-finder workflow

## Definition of done

All 8 unit tests green. TCP16 probe surfaced in DiagnosticsScreen Tools section with per-ASN result table. `runWithRttHint` exposed for SNI-finder reuse.
