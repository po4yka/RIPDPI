...
- [ ] #task Add Pluggable Transport (obfs4 / Snowflake / meek) Reachability Probe #repo/RIPDPI #area/diagnostics #status/doing 🔼

## Objective

Add `PluggableTransportReachabilityProbe` that checks whether each pluggable transport endpoint class — obfs4 bridges, Snowflake brokers + STUN, meek front domains — is reachable from the user's network. Reports per-PT verdict matrix: `PT_OK`, `PT_BRIDGE_BLOCKED`, `PT_BROKER_BLOCKED`, `PT_FRONT_BLOCKED`. Surfaces a rich signal about which transport endpoint classes are available on the local network, even though RIPDPI doesn't ship PTs as transports.

## Context

Pluggable transports expose distinct endpoint and handshake patterns. Each one provides a different network-path signal:

| PT | Transport shape | Detection signature |
|---|---|---|
| **obfs4** | Random-looking bytes + IAT timing; pre-shared cert | Bridge IPs are public; reachability tests whether bridge endpoints are reachable |
...
A user with all 3 unavailable is on a constrained network. A user with snowflake working but obfs4 unavailable has endpoint filtering, not protocol filtering. A user with meek working but the others unavailable has HTTPS CDN reachability while the other PT endpoint classes are unavailable.

Even though RIPDPI doesn't bundle PTs, this probe is valuable as a **transport-reachability fingerprint**: it tells the user which endpoint classes are usable on the local network.

**Per-PT probe:**

1. **obfs4 reachability** — pull a small set of obfs4 bridges from the bundled `bridges.obfs4` list (refreshable via user override); for each, attempt TCP connect + first 32 bytes of obfs4 handshake; verdict `PT_OK` if any bridge accepts initial bytes, else `PT_BRIDGE_BLOCKED`
...
**No actual PT session traffic is sent.** This is a *reachability* probe, not a usage probe — it tests whether the endpoints respond, not whether sessions work end-to-end.
...
- [ ] obfs4 probe: TCP connect to bridge IP:port, send 32 bytes of mock-obfs4 handshake (no real cert exchange or full session), wait for any TCP response; success = bridge sent ≥1 byte
...
- [ ] No full PT sessions, no control port, no real obfs4 cert validation — strictly a reachability fingerprint
...
- [ ] Privacy: this probe makes outbound connections to public PT endpoints. Privacy Mode disables it; settings entry under "Diagnostic — advanced" with a clear explainer that the probe contacts the broker and obfs4 bridges
...
All 9 unit tests green. PT reachability surfaced in DiagnosticsScreen Tools as "Pluggable Transport Reachability" card with 3 per-PT rows. Privacy explainer present. No full PT session traffic generated. Bridge list refreshable via user override.
