...
Most of the actual bypass-client user base outside Russia is in Persian-, Chinese-, and Arabic-speaking regions. Shipping EN+RU only is a hard adoption barrier in those geographies. Picking a translation pipeline now also prevents the one-off-PR-per-language chaos other projects suffer.
...
- **First wave targets user-geography match:** zh-CN, fa (Persian), ar (Arabic), de, es, fr. These cover ~70% of realistic non-RU users sampled from community chat demographics.
...
- **In scope:** translation pipeline selection and setup, `values-zh-rCN`, `values-fa`, `values-ar`, `values-de`, `values-es`, `values-fr` initial wave; `translatable="false"` audit on existing strings; right-to-left layout verification for Arabic and Persian.
...
- [ ] `values-zh-rCN`, `values-fa`, `values-ar`, `values-de`, `values-es`, and `values-fr` directories exist and cover ≥95% of `values/` strings.
- [ ] RTL layout renders correctly in fa and ar (screenshot tests under Roborazzi cover the main screens in each).
