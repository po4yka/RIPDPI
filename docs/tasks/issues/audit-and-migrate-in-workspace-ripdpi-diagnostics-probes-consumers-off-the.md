...
Context ripdpi-diagnostics-probes is now a compat facade for external consumers. In-workspace callers should depend directly on the narrower ripdpi-diagnostics-* crates so the compat-facade feature can eventually be marked external-only.
...
Definition of done PR merged; QA Lead confirms inventory in POY-4 closure note.
