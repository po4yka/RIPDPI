...
- 2026-08-28: `4d6f848bc` verifies against pinned `anytls-go` commit `2012ef89768409f45437f1c06a7af5f6eea402ad`: 64 KiB TCP payload, sibling stream closure, UDP-over-TCP datagrams of 0/1/1200/8192 bytes, and wrong-password rejection. Larger UDP payload support is not claimed. Session tasks are owned and drained on stop; canceled writes cannot leave partial protocol frames. `4fddd82ec` adds masked AnyTLS password, SNI and UDP controls to the main Mode Editor, preserves imported credential fields only while profile identity/kind match, and prevents stale hidden credentials from returning after rebinding. Targeted editor/persistence tests and unchanged Mode Editor screenshot families pass. The former offline-only and inline-editor deferrals are resolved by this code; task closure still requires exact-commit hosted CI.
