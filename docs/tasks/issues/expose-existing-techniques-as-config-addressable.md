...
- 2026-05-10 current checkout: `./gradlew :app:connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.poyka.ripdpi.jni.StrategyEngineJniInstrumentedTest -Pripdpi.localNativeAbis=arm64-v8a` passed on `emulator-5554` (`Pixel_10_Pro(AVD) - 17`) and validated that Android JNI accepts registry YAML containing `type: fake`, `type: udplen`, and `type: ipv6Ext`.
- Added `NativeBridgeInstrumentedTest#rawBindingsAcceptZapretEgressStrategyTunnelConfig` to prove the Android tunnel JNI `create()` path accepts a serialized `Tun2SocksConfig` carrying `strategyChainYaml` with `type: fake`, `type: udplen`, `type: ipv6Ext`, and `protectPath`.
...
- Added rooted-device runtime wiring for raw packet injection. `type: fake` now executes raw packet sends through `ripdpi-runtime-platform`, which tries the registered root-helper socket before falling back to direct app-process raw sockets; `Tun2SocksConfig.rootHelperSocketPath` is supplied only when `rootModeEnabled` is true. Verification: `CARGO_TARGET_DIR=/Users/po4yka/GitRep/.codex-targets/ripdpi-root-helper-raw cargo test --manifest-path native/rust/Cargo.toml -p ripdpi-tunnel-core --locked tun_egress_interceptor`; `./gradlew :core:service:ktlintCheck :core:engine:ktlintCheck :core:service:testDebugUnitTest --tests com.poyka.ripdpi.services.RipDpiVpnServiceConfigTest --tests com.poyka.ripdpi.services.VpnTunnelRuntimeTest -Pripdpi.skipNativeBuild=true`. Still blocked until a rooted Android target proves live raw egress.
- Android-target verification for the cfg-gated root-helper client path passed with `ANDROID_NDK_HOME="$HOME/Library/Android/sdk/ndk/29.0.14206865" cargo check --manifest-path native/rust/Cargo.toml -p ripdpi-runtime-platform --target aarch64-linux-android --locked` using the matching NDK LLVM toolchain on `PATH`.
