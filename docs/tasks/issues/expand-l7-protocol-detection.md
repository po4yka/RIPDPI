...
- Added realistic WireGuard initiation and DTLS ClientHello sample-payload tests so classifier coverage no longer relies only on zero-filled or minimal-prefix fixtures.
- Verification: `cargo fmt --manifest-path native/rust/Cargo.toml --all --check`; `CARGO_TARGET_DIR=/Users/po4yka/GitRep/.codex-targets/ripdpi-protocol-detect-samples cargo test --manifest-path native/rust/Cargo.toml -p ripdpi-protocol-detect --locked`.
