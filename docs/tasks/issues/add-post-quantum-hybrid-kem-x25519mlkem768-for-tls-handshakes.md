...
- 2026-06-05: Re-verified all criteria. Criterion 2 confirmed: `CHROME_LATEST` and `CHROME_DESKTOP_STABLE` hardcode `curves: "X25519MLKEM768:X25519:P-256:P-384"` and `key_share_profile: "x25519mlkem768_x25519"`; `packet_parity_tests.rs` asserts X25519_MLKEM768 (0x11ec) is first in supported_groups and key_share for chrome/edge profiles. Criterion 3 partial: ripdpi-vless (`reality.rs:46`), ripdpi-xhttp (`connect.rs:31`), and ripdpi-masque all depend on ripdpi-tls-profiles and route through `configure_builder`/`build_connector` → `apply_profile` → `set_curves_list(config.curves)`, so X25519MLKEM768 is propagated; however criterion 1's `kem_groups` config option (the thing they should "honour") does not exist in `ProfileConfig` (`profile.rs`), so criterion 3 cannot be satisfied independently. Criteria 1, 3, 4, 5 remain unimplemented; status remains `doing`.
