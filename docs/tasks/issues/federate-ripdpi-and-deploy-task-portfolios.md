...
- [ ] Repository differences are declared in a checked-in project config rather
      than hard-coded in the shared tasking implementation.
- [ ] `parent`, `blocked_by`, and `related_tasks` accept qualified references of
      the form `owner/repository#TASK-ID`; legacy `linked_task` is rejected.
- [ ] `taskctl export --json` and `taskctl federation list|ready|graph|validate`
      expose a versioned two-repository graph with derived reverse edges.
- [ ] Strict federation rejects missing peers/tasks, contract drift, cross-repo
      cycles, and dropped blockers while resolving completed blockers from Git
      terminal history.
- [ ] RIPDPI task contracts, generated board, unit tests, and clean-install
