...
- [~] One Criterion benchmark per transport that drives a loopback pair through a representative payload size (1 MiB). **Partial**: `protocol_throughput.rs` covers VLESS+Reality, VLESS-over-xHTTP-over-Reality, ShadowTLS v3, and MASQUE (H2 CONNECT-TCP) — driven against `VlessRealityLoopback` / `XhttpRealityLoopback` / `ShadowTlsLoopback` / `MasqueH2ConnectUdpFixture`. (MASQUE was unblocked by adding a `root_certificate_pem` trust-anchor option to `MasqueConfig` — cert pinning, verification stays ON — plus a fixture cert getter.) Deferred (blocked on enabling work, not a fixture gap): Hysteria2/TUIC need a QUIC proxy-server loopback (not the generic `QuicLoopback` echo); WS-tunnel needs an async client — `ripdpi-webtunnel`'s `connect_webtunnel` returns a synchronous boring `SslStream<std::net::TcpStream>` that cannot feed the async `roundtrip`/`tokio::io::split` harness.
