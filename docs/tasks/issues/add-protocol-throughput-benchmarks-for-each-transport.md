...
- [~] One Criterion benchmark per transport that drives a loopback pair through a representative payload size (1 MiB). **5 of 7 covered**: `protocol_throughput.rs` covers VLESS+Reality, VLESS-over-xHTTP-over-Reality, ShadowTLS v3, MASQUE (H2 CONNECT-TCP), and WS-tunnel (WebTunnel) — driven against `VlessRealityLoopback` / `XhttpRealityLoopback` / `ShadowTlsLoopback` / `MasqueH2ConnectUdpFixture` / `WebTunnelFixture`. (MASQUE was unblocked by adding a `root_certificate_pem` trust-anchor option to `MasqueConfig` — cert pinning, verification stays ON — plus a fixture cert getter; WS-tunnel was unblocked by adding an async client `connect_webtunnel_async` to `ripdpi-webtunnel`, since the sync `connect_webtunnel` returns a `std`-I/O boring `SslStream` that cannot feed the async harness.) Deferred: Hysteria2/TUIC need a QUIC protocol-server loopback (not the generic `QuicLoopback` echo). Hysteria2 was attempted — a quinn + `h3::server` loopback that auths (233) and accepts the raw proxy stream works, but the client closes its own QUIC connection right after auth (h3 graceful shutdown → `H3_NO_ERROR`), killing the proxy round-trip; see [[investigate-hysteria2-client-closes-quic-connection-after-auth]]. TUIC needs its own loopback (TLS keying-material-export auth + command framing).
