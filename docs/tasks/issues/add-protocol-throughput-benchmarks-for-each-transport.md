...
- [~] One Criterion benchmark per transport that drives a loopback pair through a representative payload size (1 MiB). **Partial**: `protocol_throughput.rs` covers VLESS+Reality, VLESS-over-xHTTP-over-Reality, and ShadowTLS v3 (driven against `VlessRealityLoopback` / `XhttpRealityLoopback` / `ShadowTlsLoopback`). Deferred (each blocked on enabling work, not a fixture gap): Hysteria2/TUIC need a QUIC proxy-server loopback (not the generic `QuicLoopback` echo); MASQUE needs a non-test trust path — the H2 fixture mints an unexposed ephemeral self-signed cert and the client only relaxes TLS verification under `#[cfg(test)]`, so an external bench crate hits `CERTIFICATE_VERIFY_FAILED` (fix: a `root_certificate_pem` trust-anchor option on `MasqueConfig`, mirroring Trojan/AnyTLS, + a fixture cert getter); WS-tunnel needs an async client — `ripdpi-webtunnel`'s `connect_webtunnel` returns a synchronous boring `SslStream<std::net::TcpStream>` that cannot feed the async `roundtrip`/`tokio::io::split` harness.
