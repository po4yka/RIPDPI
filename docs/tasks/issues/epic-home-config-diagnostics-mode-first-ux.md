...
- Config tab is split into two sub-sections: **Local Path Optimization** (desync method, listen address, DNS, mode) and **VPN** (outbound relay, server credentials, protocol).
...
| 4 | `wire-home-local-path-optimization-card-actions` | Local bypass enable/disable + navigate to config |
...
- Toggling Local Path Optimization or VPN from the Home card starts/stops the respective engine path.
