...
- 2026-07-22: Added a source-owned dual-vantage PCAP oracle. It binds both raw captures to canonical producer metadata and a strict private ledger, parses Ethernet/raw-IP/SLL/SLL2 plus bounded VLAN stacks, aligns windows by packet marker order without mixing local and remote clocks, derives counters itself, emits redacted mode-0600 unstamped observations, and rejects malformed, truncated, aliased, stale, or caller-verdict inputs. The generic marker seam is forbidden for every real Android dual-vantage gate, so no producer allowlist or release PASS was opened. Gate-specific Android action semantics, a test-APK provenance chain, and the ordinary raw verifier remain blocking.
