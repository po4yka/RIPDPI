...
- `ripdpi-masque` now preserves configured endpoint paths for HTTP/3 and HTTP/2, reports HTTP/3 to HTTP/2 fallback in telemetry, and classifies Cloudflare direct auth failures by their actual cause; Track S now keeps a repo-owned field-validation baseline in `docs/native/relay-masque-field-validation.md`.
