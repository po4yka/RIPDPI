...
Stage runners are registered by a **hand-written `Vec`** in
`execution_coordinator()` at
`ripdpi-monitor-engine/src/engine/runners/mod.rs`:
...
`impl_connectivity_runner!` macro in `engine/runners/connectivity/mod.rs`,
