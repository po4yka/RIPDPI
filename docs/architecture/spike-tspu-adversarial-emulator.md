...
`test-lab/chaos/` ships Toxiproxy + netem for packet loss, latency, and jitter. None of those reproduce what TSPU and similar middleboxes actually do to a flow: RST-injection on SNI match, blackhole after N bytes, selective drop of QUIC Initials, MTU-clamp. Packet-smoke today verifies the byte shape on the wire but never against an adversary that *reacts* to that shape. A desync mode can pass packet-smoke and still fail in the field because no part of CI exercises it against the actual middlebox behaviour we are designed to evade.
...
1. **rst-after-sni-match.** Inspect TLS ClientHello; if the SNI matches a configured pattern list, inject a TCP RST toward the client and silently drop subsequent segments. Models classic RU-TSPU SNI block.
...
- Not a substitute for real-provider testing. The emulator codifies *known* adversary patterns; real TSPU has surface area beyond what we enumerate. Phase-16 matrix on real SIM remains the release gate.
...
1. v1 land patterns 1 (rst-after-sni-match) and 4 (quic-initial-drop) only. These cover the bulk of observed RU-TSPU behaviour and let us validate the harness shape before adding more.
