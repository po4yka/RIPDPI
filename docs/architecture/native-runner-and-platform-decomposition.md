...
- `ripdpi-diagnostics-probes`: aggregate probe crate retained with root-level exports for the current probe descriptors and adapters. The `compat-facade` feature still exists, but its `compat` module is empty today; current in-tree consumers use the root exports and descriptor table directly.
...
- Keep `ripdpi-diagnostics-probes` alive as a versioned aggregate probe surface. `ripdpi-monitor-engine` and `ripdpi-android` no longer depend on it directly, but `ripdpi-diagnostics-runner` and `feature-contract-harness` still use the crate for probe adapters, descriptors, and contract checks.
...
2. Diagnostics facade narrowing is partial, not complete. `Cargo.toml` for `ripdpi-monitor-engine` and `ripdpi-android` no longer references `ripdpi-diagnostics-probes`, but `ripdpi-diagnostics-runner` and `feature-contract-harness` remain in-tree consumers. The crate's `lib.rs` currently exports the probe descriptors and concrete probe adapters at the root regardless of `compat-facade`; the default feature gates only an empty `compat` module.
...
- **Aggregate facade drift (medium-low).** Because in-tree consumers still use `ripdpi-diagnostics-probes`, the crate remains load-bearing. If those consumers migrate away later, the root export surface and empty `compat` module should be reassessed instead of assuming they are only an external-compatibility promise. See follow-up FU-1.
...
- **FU-1.** Audit internal and external consumers of `ripdpi-diagnostics-probes`. The current internal consumers are `ripdpi-diagnostics-runner` and `feature-contract-harness`; if they migrate away, decide whether the aggregate crate remains a documented public compatibility surface or should be retired.
- **FU-2.** Add a `#[deny(unused_imports)]`/`cargo doc --no-deps` smoke for `ripdpi-diagnostics-probes` with `--no-default-features` in CI to prevent silent rot of the root export surface and the compat-facade-off path.
