...
This file keeps the path expected by `test-lab/scripts/check-feature-test-signoff.sh`, while the evidence below reflects the current 2026-05-19 local QA pass through commit `ded3ac55` with the connected non-rooted Pixel 8 Pro and local lab tooling. The `RIPDPI_Root_API34` AVD is available and shell-rooted, but the opt-in root-helper smoke still skips because that AVD does not grant executable `su` to the app UID.
...
| Git commit | `ded3ac55` after local QA fixes |
...
| Root Helper and Privileged Operations | Partial | Root-helper manager fallback unit coverage passed, the opt-in rooted instrumentation smoke compiled and skipped safely on the shell-rooted API 34 AVD, and readiness correctly reports the Pixel as non-rooted. | Rooted physical device or app-granting Magisk/SuperSU emulator helper extraction, privileged action, cleanup, and redaction evidence remain. |
...
| Test-Lab and Automation Tooling | Partial | Readiness, signoff guard, artifact path checks, Appium, Maestro, physical packet smoke, and adversarial middlebox dry-run all executed locally. | Routed netem VM/router carrying Pixel traffic and remote local-network-lab workflow evidence remain. |
...
1. Run rooted physical device checks on a rooted phone, or run the opt-in root-helper smoke on an emulator with app-granting Magisk/SuperSU root: root access detection, root-mode gating, helper extraction/readiness, privileged action artifact, cleanup, and log redaction.
