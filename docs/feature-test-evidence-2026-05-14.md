...
| App shell, navigation, and settings | Partial | API 34 fresh-start UI smoke reached onboarding, Home, and Settings; API 35 Persian large-font forced-RTL smoke rendered Home, Config, Diagnostics, and Settings; connected UI tests passed with no failures; focused Home, Settings, Onboarding, shell-controller, and settings-state unit tests passed | Manual TalkBack, dynamic-color contrast, process-kill and migrated-install checks |
| Proxy service | Partial | Service lifecycle connected tests passed, including resolved command-line preferences; physical proxy smoke proved local SOCKS5 readiness plus DNS/HTTP/HTTPS/TCP/UDP through the lab; host `curl --socks5-hostname` returned HTTP 200 through `adb forward`; API 27/31/35 emulator proxy smokes passed the same lab probes; focused proxy supervisor, startup-failure, runtime-coordinator, auto-apply, JSON, and preference tests passed | Port conflict on device and relay-on proxy combinations |
