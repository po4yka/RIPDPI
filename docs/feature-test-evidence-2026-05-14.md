...
This file keeps the path expected by `test-lab/scripts/check-feature-test-signoff.sh`, while the evidence below reflects the current 2026-05-19 local QA pass through commit `af4891d5` with the connected non-rooted Pixel 8 Pro and local lab tooling. The `RIPDPI_Root_API34` AVD is now Magisk-rooted with app-granting `su`, and the opt-in root-helper smoke passes there; the rooted physical-device release row remains open because the attached Pixel 8 Pro is intentionally non-rooted.
...
| Git commit | `af4891d5` after local QA fixes |
