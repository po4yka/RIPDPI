...
| Fix all issues found during the local pass | QA commits through `e6c6fefd`, including physical packet-smoke, readiness, root-helper launch fallback, macOS host UDP/DNS, VPN readiness-contract fixes, root-helper smoke diagnostics, Docker netem readiness reporting, root-helper IPC lifecycle hardening, Android network E2E stabilization, diagnostics scan gating, mutation harness fixture exposure, TSPU dry-run command repair, signoff Python detection hardening, and macOS SDK resolution for emulator helpers | Local findings were fixed and verified where the required device or lab was available. |
...
| Verify remote release gates | PR #117 check rollup plus required workflow list | Partial: PR #117 was green at `e6a57057`, then advanced to `0ad430c8` after merging current `origin/main`; current-head checks were still in progress before the local `e6c6fefd` evidence update was pushed. Merge-to-main confirmation plus local-network-lab, offline analytics, mutation-testing, and Fuzz Nightly evidence remain. |
...
| Fix all issues found during the local pass | QA commits through `e6c6fefd` and retest commands | Covered locally | None for locally reproducible issues with the currently available lab/device set |
...
| Verify remote release gates | PR #117 check rollup, branch state, and workflow list | Partial | Current-head PR checks were still in progress after the branch merged current `origin/main`; merge-to-main confirmation plus local-network-lab, offline analytics, mutation-testing, and Fuzz Nightly remote workflow results remain |
