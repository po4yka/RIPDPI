...
| Verify local artifacts referenced by the evidence ledger exist | `test-lab/scripts/test-feature-artifact-paths.sh`; connected-test XML, debug APK, release APK, and `doctor.json` exist | Covered locally | None for current artifact-path audit; preserve or archive artifacts before cleanup |
...
- Local post-commit checks: `git diff --check`, `cargo fmt --check`,
  ...
  cells, required `manual` readiness rows, missing required readiness rows,
  duplicate required rows, invalid statuses, and malformed readiness JSON, the
  relay matrix config self-test including template parity,
