...
Russian middlebox drops UDP/443 packets >1001 bytes containing QUIC v1 at offset 1.
`build_realistic_quic_initial()` pads to `QUIC_FAKE_INITIAL_TARGET_LEN = 1200` bytes — this trips the middlebox filter.
...
fall back to CompatDefault (620 bytes). Russian middlebox drops UDP/443
