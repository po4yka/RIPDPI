...
**Goal:** Implement three P1 improvements to RIPDPI's network optimization: (1) QUIC fake packet size cap ≤1000 bytes to avoid Russian TSPU 1001-byte threshold, (2) QUIC→TCP fallback cache so repeated QUIC failures cause the proxy to bypass QUIC desync, and (3) `ThrottlingFreeze` failure detection for the 2025 Russian ISP TCP freeze pattern.
